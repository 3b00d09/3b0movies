<script>
    ...
    onMount(async() =>{
        const url = `https://api.themoviedb.org/3/discover/movie?api_key=${ENV_API_KEY}&language=${ENV_API_LANG}&region=${ENV_API_REGION}&page=${currPage}&with_genres=28`
        ...
        // function that runs when we receive a dispatch from child component, we use it to load more movies from API
    async function fetchMore(movies, currPage){

            const url = `https://api.themoviedb.org/3/movie/popular?api_key=${ENV_API_KEY}&language=${ENV_API_LANG}&region=${ENV_API_REGION}&page=${currPage}`
