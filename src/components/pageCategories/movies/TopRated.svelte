<script>
    ...
    onMount(async() =>{
        ...
        const url = `https://api.themoviedb.org/3/movie/top_rated?api_key=${ENV_API_KEY}&language=${ENV_API_LANG}&region=${ENV_API_REGION}&page=${currPage}`
    ...
    async function fetchMore(movies, currPage){
        ...
        let tempArray =  [...APIreturn.results.filter(movie => movie.poster_path)]
    
    
        let i = currPage + 4;
        while(tempArray.length > 0){
            let temp = "page" + i
            Newpages.set(temp, tempArray.splice(0,moviesPerSlide))
            i++;
        }

        Newpages.forEach((key) =>{
            movies.push(key)
        })
